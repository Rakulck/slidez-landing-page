...
function useTypewriter(
  ...
  const tick = useCallback(function tick() {
    const current = HERO_PROMPTS[promptIdx.current];
    ...
    if (!deleting.current) {
      ...
      if (charIdx.current === 0) {
        ...
        promptIdx.current = (promptIdx.current + 1) % HERO_PROMPTS.length;
...
export default function Hero() {
  ...
  // Preload prompt slides and model photos for instant smooth crossfades
  useEffect(() => {
    PROMPT_PAUSE_IMAGES.forEach((src) => {
      const img = new window.Image();
      img.src = src;
    });
    PROMPT_TYPING_MODELS.forEach((m) => {
      const model = getHeroModel(m.gender, m.modelId);
      const img = new window.Image();
      img.src = model.src;
    });
  }, []);
